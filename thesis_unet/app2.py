# app2.py
# Sen2Fire U-Net — Pixel-wise Segmentation → Sigmoid → Data-driven Threshold (val-derived) → Visualization/Product
# No sidebar, centered top upload panel, human-style layout. Ground-truth removed.
...
# PLACEBO for now. Replace later with JP's printed values (len=13 each).
MEAN_13 = [0.0] * 13
STD_13 = [1.0] * 13

# Validation-derived threshold (data-driven from sweep on val)
T_BEST = 0.15

# Fixed RGB composite mapping (display only)
RGB_IDXS = (0, 1, 2)
...
def load_model(weights_path: str):
    """
    Robust loader for PyTorch 2.6+ (torch.load defaults weights_only=True).
    We explicitly set weights_only=False if supported, because the checkpoint is trusted.
    """
    device = get_device()
...
def load_npz_from_bytes(npz_bytes: bytes):
    ...
    Required keys:
      ...
      aerosol: (H,W)
    Optional:
      label: (H,W)  -> ignored in this version
    Returns:
      x13: (13,H,W) float32
    """
    ...
    aer = aer[None, ...]  # (1,H,W)
    x13 = np.concatenate([img, aer], axis=0)  # (13,H,W)
    return x13


def to_rgb_for_display(x13: np.ndarray, rgb_idxs=RGB_IDXS):
    """
    x13: (13,H,W) raw (NOT normalized) values.
    Create an RGB composite by selecting 3 channels and percentile-scaling.
    """
    rgb = np.stack([x13[rgb_idxs[0]], x13[rgb_idxs[1]], x13[rgb_idxs[2]]], axis=-1)  # (H,W,3)
...
# PAGE CONFIG + STYLING
# =========================
st.set_page_config(page_title="Sen2Fire U-Net (Pixel-wise)", layout="wide")
...
# HEADER
# =========================
st.title("Sen2Fire U-Net — Transparent Pixel-wise Inference")
st.markdown(
    "Segmentation (logits) → sigmoid → data-driven threshold (validation-derived) → visualization & product."
)
...
if MEAN_13 == [0.0] * 13 and STD_13 == [1.0] * 13:
    st.warning(
        ...
        "This is OK for UI/pipeline wiring. Replace with JP’s MEAN_13/STD_13 later for accurate calibration."
    )

# =========================
# CENTERED INPUT (NO SIDEBAR)
# =========================
...
# This centers the uploader itself (Streamlit still renders internally, but this helps visually)
...
# Centered key-value info (still aligned, but not “Word-left”; it sits inside a centered container)
st.markdown(
    f"""
<div class="kv-center">
...
x_t = torch.from_numpy(x13).unsqueeze(0).to(device)  # (1,13,H,W)
...
Notes: This app runs pixel-wise inference only. Any patch-level aggregation, area-based categorization, or risk mapping is intentionally removed to match the latest methodology.
</div>
""",
    unsafe_allow_html=True,
)